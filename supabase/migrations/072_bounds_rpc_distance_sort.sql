-- 072: bounds RPC에 거리순 정렬 추가 (p_user_lat, p_user_lng 파라미터)
...
CREATE OR REPLACE FUNCTION search_restaurants_bounds_simple(
  ...
  ORDER BY
    CASE WHEN p_sort = 'distance' AND p_user_lat IS NOT NULL AND p_user_lng IS NOT NULL
         THEN ST_Distance(rst.geom, ST_SetSRID(ST_MakePoint(p_user_lng, p_user_lat), 4326)::geography)
    END ASC,
    CASE WHEN p_sort = 'name' OR (p_sort = 'distance' AND p_user_lat IS NULL)
         THEN rst.name
    END ASC,
    rst.name ASC
...
CREATE OR REPLACE FUNCTION search_restaurants_bounds_auth(
  ...
  ORDER BY
    CASE WHEN p_sort = 'distance' AND p_user_lat IS NOT NULL AND p_user_lng IS NOT NULL
         THEN ST_Distance(sub.geom, ST_SetSRID(ST_MakePoint(p_user_lng, p_user_lat), 4326)::geography)
    END ASC,
