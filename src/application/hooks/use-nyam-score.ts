...
export function useNyamScore(
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      ...
      setPrediction(null)
      try {
        const result = await predictionRepo.predictScore(userId, itemId, category)
  ...
  }, [itemId, category, userId, startTransition])

  return { prediction, isLoading: isPending, error }
}
