...
export function NaverImportSheet({
  ...
  return (
    <BottomSheet isOpen={isOpen} onClose={handleClose} title="네이버 지도 가져오기" maxHeight="85vh">
      {/* Step 1: URL 입력 */}
      {(status === 'idle' || status === 'fetching' || status === 'error') && (
        <div className="flex flex-col gap-3">
          ...
          <div className="rounded-xl px-4 py-3" style={{ backgroundColor: 'color-mix(in srgb, var(--primary) 8%, transparent)' }}>
      ...
      {status === 'previewing' && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ backgroundColor: 'color-mix(in srgb, var(--accent-food) 15%, transparent)' }}>
              <MapPin size={16} style={{ color: 'var(--accent-food)' }} />
            </div>
            <div>
              <p style={{ fontSize: '15px', fontWeight: 700, color: 'var(--text)' }}>{folderName}</p>
              <p style={{ fontSize: '12px', color: 'var(--text-hint)' }}>{places.length}개 장소</p>
            </div>
          </div>

          <div className="flex flex-col rounded-xl" style={{ border: '1px solid var(--border)', maxHeight: '300px', overflow: 'auto' }}>
            {places.map((place, i) => (
              <div key={place.naverPlaceId} className="flex items-center gap-3 px-4 py-2.5" style={{ borderBottom: i < places.length - 1 ? '1px solid var(--border)' : undefined }}>
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full" style={{ backgroundColor: 'var(--bg)', fontSize: '11px', color: 'var(--text-hint)', fontWeight: 600 }}>
                  {i + 1}
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate" style={{ fontSize: '14px', color: 'var(--text)' }}>{place.name}</p>
                  <p className="truncate" style={{ fontSize: '11px', color: 'var(--text-hint)' }}>{place.address}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Step 3: 가져오는 중 / Fetching */}
      {(status === 'importing' || status === 'fetching') && (
        <div className="flex flex-col items-center gap-3 py-8">
          <Loader2 size={32} className="animate-spin" style={{ color: 'var(--primary)' }} />
          <p style={{ fontSize: '15px', color: 'var(--text)' }}>
            {status === 'importing' ? `${places.length}개 식당을 가져오는 중...` : '목록을 불러오는 중...'}
          </p>
          {status === 'importing' && (
            <p style={{ fontSize: '13px', color: 'var(--text-hint)' }}>잠시만 기다려주세요</p>
          )}
        </div>
      )}
      ...
      {status === 'done' && result && (
        <div className="flex flex-col items-center gap-4 py-4">
          <div className="flex h-14 w-14 items-center justify-center rounded-full" style={{ backgroundColor: 'color-mix(in srgb, var(--positive) 15%, transparent)' }}>
            <CheckCircle2 size={28} style={{ color: 'var(--positive)' }} />
          </div>
          <p style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text)' }}>가져오기 완료</p>
          <div className="w-full rounded-xl px-4 py-3" style={{ backgroundColor: 'var(--bg)' }}>
