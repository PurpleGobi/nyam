...
interface GridItem {
  ...
  trendKey?: keyof ActivitySummary
}

const GRID_ITEMS: GridItem[] = [
  { key: 'restaurantVisits', label: '식당 방문', icon: Utensils, color: 'var(--accent-food)', format: (v) => `${v}곳`, trendKey: 'restaurantThisMonth' },
  { key: 'wineTastings', label: '와인 시음', icon: Wine, color: 'var(--accent-wine)', format: (v) => `${v}병`, trendKey: 'wineThisMonth' },
...
export function OverviewGrid({ summary }: OverviewGridProps) {
  return (
    <div className="mx-4 grid grid-cols-2 gap-3">
      {GRID_ITEMS.map((item) => {
        const trendValue = item.trendKey ? summary[item.trendKey] : 0
        return (
          <div
            key={item.key}
          ...
          >
            ...
            <div className="min-w-0">
              ...
              {item.trendKey && trendValue > 0 && (
                <p style={{ fontSize: '10px', color: 'var(--positive)' }}>
                  +{trendValue} 이번 달
                </p>
              )}
            </div>
