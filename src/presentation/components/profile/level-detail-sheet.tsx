...
import { Target, Sparkles, Repeat, Award, Trophy, Heart } from 'lucide-react'
...
export function LevelDetailSheet({
  ...
  const sheetTitle = `${AXIS_LABELS[axisType ?? 'category']} · ${axisValue ?? '-'}`

  return (
    <BottomSheet isOpen={isOpen} onClose={onClose} title={sheetTitle} maxHeight="70dvh">
      ...
      {levelInfo ? (
        <div className="flex flex-col gap-5">
          ...
          <div>
            ...
            <div className="mt-1.5 h-2 overflow-hidden rounded-full" style={{ backgroundColor: 'var(--bg)' }}>
              <div
                ...
                style={{ width: `${Math.min(levelInfo.progress * 100, 100)}%`, backgroundColor: levelInfo.color }}
              />
            </div>
          </div>

          {/* 통계 3열 */}
          <div className="flex gap-2 rounded-xl px-3 py-3" style={{ backgroundColor: 'var(--bg)', border: '1px solid var(--border)' }}>
          ...
          {/* XP 구성 */}
          <div>
            <p className="mb-2" style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text)' }}>XP 구성</p>
          ...
          {nextMilestone && (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)' }}>{nextMilestone.milestone.label}</p>
        ...
        <div className="flex flex-col items-center py-8">
          <p style={{ fontSize: '14px', color: 'var(--text-hint)' }}>아직 경험치가 없습니다</p>
        </div>
      )}
    </BottomSheet>
  )
