...
import { X, Users, MapPin, Heart, Check } from 'lucide-react'
...
export function JoinFlow({
  ...
  return (
    <div className="bottom-sheet-overlay flex items-end justify-center" style={{ zIndex: 50 }} onClick={onClose}>
      <div className="bottom-sheet w-full max-w-[480px] pb-8" style={{ position: 'relative' }} onClick={(e) => e.stopPropagation()}>
        {/* 헤더 */}
        <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid var(--border)' }}>
          <span style={{ fontSize: '15px', fontWeight: 700, color: 'var(--text)' }}>
            {isClosed ? '버블 팔로우' : `${bubble.name}에 가입하시겠어요?`}
          </span>
        ...
        {/* 에러 메시지 */}
        {eligibilityError && (
          <p className="mx-4 mt-3 text-center text-[13px]" style={{ color: 'var(--negative)' }}>
            {eligibilityError}
          </p>
        )}
