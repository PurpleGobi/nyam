...
export function PendingApprovalList({ members, onApprove, onReject, onMemberClick, maxPreview = 3, onViewAll, hideEmptyMessage }: PendingApprovalListProps) {
  if (members.length === 0) {
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-[13px] font-semibold text-[var(--text)]">
                {m.nickname}
                {m.handle && <span className="ml-1 text-[11px] font-normal text-[var(--text-hint)]">@{m.handle}</span>}
