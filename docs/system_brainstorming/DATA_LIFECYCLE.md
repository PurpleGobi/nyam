...
| Step 1/3 — 기록 | 지역 선택 (6개 시드: 을지로, 광화문, 성수, 강남, 홍대, 이태원) + 식당 등록(검색→선택) | `users.preferred_areas` + `records` (status='checked', 이름만) | 평가 없이 등록만 → 종합 XP 0. 사분면/만족도는 나중에 RECORD_FLOW에서 보충 시 rated로 전환 |
...
| 온보딩 완료 | — | `xp_histories` (bonus_onboard +10, bonus_first_bubble +5) | 온보딩 중 최대 +15 XP. bonus_first_record(+5)는 온보딩 후 첫 rated 기록 시 트리거. 6종 넛지 시작 (location/time/photo/unrated/new_area/weekly) |
...
|--------|-------------|-------------|-----------|
| L1 히어로 | 사진 캐러셀(224px) + 썸네일(160×110), 찜/공유 버튼 | `record_photos` + `restaurants.photos` + `wishlists` | 내 사진 → 외부 사진 순 |
| L2 기본 정보 | 식당명, 장르·지역·가격대 메타 행 | `restaurants` (name, genre, area, price_range) | — |
...
| 뱃지 행 | 미슐랭 ★/★★/★★★, 블루리본, TV 출연 | `restaurants` (michelin_stars, has_blue_ribbon, media_appearances) | 있을 때만 표시 |
...
| 뱃지 행 | Grand Cru 등급, Vivino 평점, Wine Spectator 등 평론가 점수 | `wines` (classification, vivino_rating, critic_scores) | 있을 때만 표시 |
...
| **탐색 (Discover)** (07_DISCOVER.md) | 지역 기반 식당 탐색 카드 — 사진, 뱃지(미슐랭/블루리본), 식당명, nyam 점수, 메타(장르·지역·특징), 외부 평점(N/K/G) | `restaurants` (photos, nyam_score, genre, area, naver/kakao/google_rating, michelin_stars, has_blue_ribbon) + `records` (AVG, COUNT) | 복합 점수: 외부평점평균×0.4 + Nyam만족도×0.3 + 기록수log×0.2 + 권위뱃지×0.1. 지역칩 기반 필터, 20개씩 무한스크롤 |
...
| **프로필** — 레벨 디테일 시트 | 축별 레벨+XP+진행바, 통계(고유장소/총기록/재방문), XP 구성(새장소/재방문/마일스톤/소셜), 다음 마일스톤 | `user_experiences` + `xp_histories` + `milestones` + `user_milestones` | reason별 GROUP BY, 미달성 마일스톤 조회 |
...
| **추천 시스템** (RECOMMENDATION.md) | P1: 재방문, 상황별, 사분면(필터), 찜 리마인드, 권위, 와인 페어링, 지역 푸시 (7종, SQL) / P2: 버블 기반(신뢰 가중), 취향 유사도 CF / P3: ML 하이브리드 | `records` + `wishlists` + `restaurants` + `wines` + `ai_recommendations` + (P2) `bubble_shares` + `bubble_members` | Phase별 점진 확장. P1은 SQL만으로 충분. 버블 추천은 P2부터 |
...
| `xp_histories` | XP 이력 로그 | id, user_id, record_id, axis_type, axis_value, xp_amount, reason(15종: record_name/score/photo/full, category, social_share/like/follow/mutual, bonus_onboard/first_record/first_bubble/first_share, milestone, revisit) | FK → users, records |
...
│  탐색(Discover) ← restaurants(지역필터) + records(AVG) + 외부평점   │
...
│  기록 상세 ←─── records + record_photos + xp_histories              │
...
| bubble_members INSERT/UPDATE/DELETE (active member 변동) | `bubbles.member_count` ±1 (owner/admin/member만. follower 제외) |
...
| 매일 | `bubbles.follower_count` | bubble_members (role='follower', status='active') COUNT — 트리거 미정의, 크론으로 보정 |
