...
synced_at         TIMESTAMPTZ NULL       -- 외부 API 마지막 동기화 시점. TTL 14일 (lazy refresh: 조회 시 14일 경과 시 백그라운드 갱신)
is_closed         BOOLEAN DEFAULT false  -- 폐업 여부 (API 재호출 시 감지)
closed_at         TIMESTAMPTZ NULL
created_at        TIMESTAMPTZ DEFAULT now()
```

> **식당 정보 갱신 정책 (Lazy Refresh)**:
> - TTL: **14일**. 사용자가 조회/기록 시 `synced_at`이 14일 이상 경과했으면 카카오 API 재호출 후 UPDATE.
> - 아무도 조회하지 않는 식당은 갱신하지 않음 (API 호출 절약).
> - 재호출 시 404 또는 폐업 상태 → `is_closed = true`, `closed_at = now()`.
> - 폐업 식당의 기존 records는 유지 (기록은 보존). 신규 기록 시 "폐업된 식당입니다" 안내.
...
-- 사용자 확인/수정 추적
verified_at       TIMESTAMPTZ NULL      -- 사용자가 확인/수정한 시점 (NULL = 미확인)
verified_fields   TEXT[] DEFAULT '{}'   -- 사용자가 수정한 필드명 목록 (빈 배열 = 수정 없이 확인)
created_at        TIMESTAMPTZ DEFAULT now()
```
...
기록의 사용자 필드 수정. 기존 `update()` 메서드를 확장하여 모든 사용자 편집 가능 필드를 지원한다.

```
Input: Partial<{
  menuName, genre, subGenre, comment, visibility, scene,
  companionCount, totalCost, visitTime, pairingFood, purchasePrice,
  tags, flavorTags, textureTags, atmosphereTags,
  ratingTaste, ratingValue, ratingService, ratingAtmosphere,
  ratingCleanliness, ratingPortion, ratingBalance,
  ratingDifficulty, ratingTimeSpent, ratingReproducibility,
  ratingPlating, ratingMaterialCost
}>
Output: { success: boolean, record: FoodRecord }
```

#### PATCH /api/records/[id]/ai-analysis
AI 분석 결과 수동 수정. record_ai_analyses의 최신 행을 업데이트한다.

```
Input: Partial<{
  identifiedRestaurant: { name, matchedPlaceId, confidence }
  orderedItems: [{ name, estimatedPrice }]
  extractedMenuItems: [{ name, price }]
  wineInfo: { name, vintage, winery, origin, variety, estimatedPriceKrw, criticScore }
  wineTastingAi: { acidity, body, tannin, sweetness, balance, finish, aroma }
}>
Output: { success: boolean }
```

#### PATCH /api/records/[id]/taste-profile
맛 프로필 수동 수정. source를 'manual'로 변경한다.

```
Input: Partial<{
  spicy, sweet, salty, sour, umami, rich,
  wineAcidity, wineBody, wineTannin, wineSweetness, wineBalance, wineFinish, wineAroma
}>
Output: { success: boolean }
```

#### PATCH /api/records/[id]/journal
블로그 제목/본문 수정.

```
Input: Partial<{ blogTitle, blogContent }>
Output: { success: boolean }
```
