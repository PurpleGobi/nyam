...
```
1. Shrinkage Mean-centering: 각 유저의 보정 평균을 빼서 편향 제거
   → 후한/박한 유저 보정 (+19점 개선 검증)
   → 기록이 적을 때는 전체 평균 쪽으로 수축하여 왜곡 방지

   보정 평균: μ̂ = (유저평균 × n + 전체평균 × λ) / (n + λ)
   - n이 적으면 → 전체평균에 수렴 (편향 보정 거의 안 함)
   - n이 크면 → 유저 고유 평균에 수렴 (full mean centering)
   - λ = 10 (기록 10개쯤부터 유저 고유 평균이 지배적)
...
> **왜 raw mean centering이 아닌가?**
> 기록 2~3개로 추정한 유저 평균은 통계적으로 불안정하다 (표준오차 = σ/√n).
> MovieLens도 CF 작동을 위해 최소 5→15개 기록을 요구했다 (Harper & Konstan, 2015).
> Shrinkage는 데이터량에 따라 단순 가중평균 ↔ full mean centering 사이를
> 연속적으로 전환하여 콜드스타트 왜곡을 방지한다.
...
예측X = μ̂_me_X + Σ(가중치ᵢ × (상대Xᵢ - μ̂ᵢ_X)) / Σ|가중치ᵢ|
예측Y = μ̂_me_Y + Σ(가중치ᵢ × (상대Yᵢ - μ̂ᵢ_Y)) / Σ|가중치ᵢ|

- n이 적은 유저: μ̂ ≈ 전체평균 → 편차 최소화 → 유사도 가중 평균에 수렴
- n이 충분한 유저: μ̂ ≈ 유저평균 → full mean centering 동작
```
