...
> 마지막 갱신: 2026-04-07 (DB_v4 타겟 중심 홈뷰 리팩토링)

## 프로젝트 요약
- 맛집/와인 기록 + 소셜(버블) 앱. Next.js App Router + Supabase + Clean Architecture
- 총 ~440개 TS/TSX 파일, 49개 마이그레이션, 3개 Edge Function
...
| 경로 | 파일 수 | 역할 | 상태 |
|------|---------|------|------|
| entities/ | 33 | 타입 정의 (record, restaurant, wine, bubble, xp, score, calendar, bookmark, home-target 등). grouped-target 삭제됨 | 안정 |
| repositories/ | 17 | 인터페이스 (RecordRepository, BookmarkRepository, BubbleRepository, HomeRepository 등) | 안정 |
| services/ | 18 | 순수 로직 (nyam-score, xp-calculator, bubble-share-sync, filter-matcher, score-fallback, visibility-filter, profile-visibility 등). record-grouper 삭제됨 | 안정 |
| constants/ | 1 | source-priority.ts (SOURCE_PRIORITY 중앙 상수) | 신규 |

### infrastructure/ (외부 시스템 구현체)
| 경로 | 파일 수 | 역할 | 상태 |
|------|---------|------|------|
| repositories/ | 17 | Supabase 구현체 (1:1 domain repo 매칭, supabase-bookmark-repository, supabase-home-repository 추가) | 안정 |
...
| components/ (19 dirs) | 순수 UI (add-flow, bubble, camera, charts, detail, home, record, search 등) + score-source-badge (discover 제거) | 안정, 일부 WIP |
...
## supabase/
- migrations/ (49개, 000~049): 스키마 전체 + RLS + triggers + cron + WSET 아로마 구조 + 버블 트리거 SECURITY DEFINER 수정 + lists→bookmarks 전환(049)
- functions/ (3): process-account-deletion, refresh-active-xp, weekly-ranking-snapshot

## DI 등록 현황 (container.ts)
17개 repo 모두 등록 완료: record, restaurant, wine, photo, xp, notification, bubble, follow, savedFilter, bookmark, profile, settings, comment, reaction, onboarding, userCoords(hook), home
...
- supabase/types.ts 재생성 필요 (bookmarks + home 관련 테이블 반영)
