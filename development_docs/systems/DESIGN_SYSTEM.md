...
> 모바일 퍼스트. 넓은 화면에서는 앱 셸을 중앙에 제약하여 모바일 앱처럼 유지한다.

### 설계 원칙

| 원칙 | 설명 |
|------|------|
| **모바일 셸 유지** | 넓은 화면에서도 콘텐츠를 모바일 폼팩터로 제약. 데스크톱용 멀티컬럼 리디자인 안 함 |
| **중앙 정렬** | 앱 셸은 화면 중앙, 양쪽은 `--bg-page` 배경으로 채움 |
| **점진적 확장** | 360px → 430px → 768px 순서로 자연스럽게 넓어짐. 768px 이상은 고정 |
| **헤더/FAB 앱 셸 내부** | 고정 요소(헤더, FAB)는 앱 셸 max-width 안에서만 동작 |

### 브레이크포인트

| 토큰 | 값 | 용도 |
|------|----|------|
| `--bp-mobile` | `430px` | 큰 모바일(iPhone Pro Max 등) 기준 |
| `--bp-tablet` | `768px` | 태블릿. 이 이상에서 앱 셸 max-width 고정 |

> Tailwind 기본 브레이크포인트(`sm:640`, `md:768`, `lg:1024`)를 그대로 사용한다.
> 커스텀 브레이크포인트는 추가하지 않는다.
...
| 속성 | 값 | 설명 |
|------|----|------|
| **max-width** | `430px` | 모바일 폼팩터 유지 |
| **min-height** | `100dvh` | 전체 높이 |
| **margin** | `0 auto` | 수평 중앙 정렬 |
| **background** | `var(--bg)` | 앱 배경 |
| **position** | `relative` | 내부 fixed 요소의 기준 |

> `430px`은 가장 큰 모바일 기기(iPhone 16 Pro Max: 430px) 기준.
> 태블릿/데스크톱에서 이 너비를 넘기면 어색해지므로 고정한다.

### 앱 셸 외부 배경

```css
/* md(768px) 이상에서 앱 셸 양쪽에 보이는 배경 */
body, html {
  background-color: var(--bg-page);  /* #EFECE7 — 디바이스 프레임 밖 배경 */
}
```
...
|------|----------|----------|
| 기본 (모바일) | 16px | `px-4` |
| md (768px+) | 24px | `md:px-6` |

> 앱 셸 max-width가 430px이므로 md 이상에서 패딩을 약간 늘려 여유감을 준다.

### 헤더 (AppHeader)

```
모바일:    position: sticky, width: 100%
md 이상:   position: sticky, 앱 셸 max-width 내부에서 동작
```

> 헤더는 `position: fixed` 대신 `position: sticky`를 사용하여
> 앱 셸 내부에 자연스럽게 귀속된다. (fixed는 viewport 기준이라 셸 밖으로 벗어남)

### FAB (Floating Action Button)

```
모바일:    position: fixed → position: sticky 컨테이너 내 absolute
md 이상:   앱 셸 내부에 귀속
```

> FAB도 앱 셸 내부에서만 동작해야 한다.
> `position: fixed`는 viewport 기준이므로, 앱 셸이 좁아지면 FAB가 셸 밖에 떠버린다.
> → 앱 셸에 `position: relative`를 주고, FAB는 앱 셸 기준 `position: absolute` 또는 sticky 컨테이너 활용.

### 그림자 구분선 (앱 셸 테두리)

```css
/* md 이상에서 앱 셸 좌우에 미세한 구분선 */
@media (min-width: 768px) {
  .app-shell {
    box-shadow: 0 0 0 1px var(--border),
                0 0 40px rgba(61, 56, 51, 0.06);
  }
}
```
...
```
□ body/html background → var(--bg-page)
□ (main)/layout.tsx → .app-shell 래퍼 적용
□ AppHeader → sticky (fixed 사용 시 앱 셸 내부 귀속 확인)
□ FAB → 앱 셸 내부 귀속 확인
□ 각 컨테이너 px-4 → 반응형 패딩 (선택: md:px-6)
□ 360px / 430px / 768px / 1024px 에서 레이아웃 확인
□ 다크모드에서 bg-page 배경 확인
```
...
| 하지 않음 | 이유 |
|----------|------|
| 데스크톱 멀티컬럼 | 개인 컬렉션 앱 — 모바일 폼팩터가 핵심 경험 |
| 사이드바 네비게이션 | 모바일 탭/FAB 패턴 유지 |
| 카드 그리드 확장 | 430px 셸 안에서 기존 레이아웃 그대로 |
| 폰트 크기 스케일링 | 모바일 타이포 그대로 유지 |
