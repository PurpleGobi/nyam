...
--gauge-wine-total-min: #D8D0E0;
--gauge-wine-total-max: #A050D0;
```
...
| 종합(total, 와인) | `#D8D0E0` | `#A050D0` | 밝은 보라 | 와인 만족도 = (x+y)/2 |
...
|------|-----|
| 크기 | 고정 20px |
| 색상 매핑 | 채널 기반 그라데이션 (Section 1 참조) |
...
사분면 내 점 지름: **고정 20px** (만족도에 따른 가변 크기 없음)
...
```
모바일:     position: sticky, width: 100%, 패딩 16px
md 이상:    position: sticky, 콘텐츠 max-width에 맞춰 확장
...
FAB는 콘텐츠 영역 우하단에 위치해야 함. 콘텐츠 max-width 기준으로 계산:

```css
.fab-add {
  right: max(16px, calc((100% - 960px) / 2 + 16px));
}
```

> 뷰포트가 960px 이하면 `right: 16px`, 이상이면 콘텐츠 영역 안쪽 16px.
...
```
1단계 (필수):
  □ globals.css에 content-feed / content-detail / content-auth 클래스 추가
  □ (main)/layout.tsx에 content-feed 래퍼 적용
  □ AppHeader 내부 콘텐츠 max-width 적용
  □ FAB right 위치 반응형 계산
  □ 홈 카드 뷰 → md:grid md:grid-cols-2 적용

2단계 (확장):
  □ 디테일 페이지 → content-detail 래퍼
  □ 프로필/설정 → content-detail 래퍼
  □ 로그인/온보딩 → content-auth 래퍼
  □ 버블 리스트 → md:grid-cols-2
  □ 디스커버 → md:grid-cols-2
  □ 바텀 시트 max-width
  □ 반응형 패딩 (px-4 → md:px-8 → lg:px-12)
```
