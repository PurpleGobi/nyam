<!-- updated: 2026-04-20 -->
<!-- depends_on: (없음 — 최하위 기반 문서) -->
<!-- affects: 모든 `src/presentation/components/**` + `src/app/globals.css` + `src/app/**/page.tsx` -->

# DESIGN_SYSTEM — 디자인 시스템

> affects: 모든 페이지
> 비주얼 레퍼런스: `_archive/prototype/00_design_system.html`
> 아이콘: Lucide (https://lucide.dev)
> 구현: Tailwind CSS v4 (`@theme` 디렉티브, CSS-first 설정 — `tailwind.config.ts` 없음), shadcn/ui (new-york 스타일)
> 토큰 정의: `src/app/globals.css` (`:root` + `@theme` + `[data-theme="dark"]` 블록)
> 만족도 게이지 채널 로직: 본 문서는 **토큰만 정의**. 채널 선택/보간 로직은 `systems/RECORD_SYSTEM.md` 및 `src/shared/utils/gauge-color.ts` 참조.
...
--text-inverse: #FFFFFF;   /* 컬러 배경 위 텍스트 (버튼 활성, 뱃지 등) */
```
> 순흑(#000, #111) 사용 금지. 항상 따뜻한 차콜 계열.
> `--text-inverse`는 하드코딩 `#fff` 대체용. `bg-accent-food`, `bg-accent-wine` 같이 짙은 배경 위 텍스트에 사용 (Tailwind: `text-text-inverse`).
...
> 본 문서는 **토큰만 정의**한다. 채널 선택 규칙·점수 → 색상 매핑 알고리즘은 `systems/RECORD_SYSTEM.md` 및 실제 구현 `src/shared/utils/gauge-color.ts` 참조.
...
| 채널 | min | max | 계열 | 용도 |
|------|-----|-----|------|------|
| `food` | `#C4B5A8` | `#A83E1A` | coral | 식당 X축 (음식) |
| `experience` | `#B5B0BA` | `#6B3FA0` | 보라 | 식당 Y축 (경험) |
| `total` | `#C4BCA8` | `#C87A0A` | 골드 | 식당 만족도 |
| `wine-total` | `#D8D0E0` | `#A050D0` | 밝은 보라 | 와인 만족도 |

> `gauge-color.ts`는 위 4개 채널 외에 `default` 채널도 노출한다 (`food`의 별칭, 하위 호환용).

- 실제 매핑은 5단계 `CHANNEL_STEPS`(0~20, 21~40, 41~60, 61~80, 81~100)로 처리한다 (연속 보간 아님, `shared/utils/gauge-color.ts` 참조).
- 사분면 점, 게이지 바, Circle Rating, 카드 점수 뱃지에 공통 적용.
- Glow(box-shadow)는 동일 색상 + `80`/`A6`/`CC` 알파 접미사.

> ※ `gauge-color.ts`에는 레거시 팔레트 `GAUGE_STEPS`(#E8A87C/#E8913A/#E06B20/#D4451A)와 이를 참조하는 `--gauge-1`~`--gauge-5` CSS 변수, 그리고 헬퍼 `getGaugeCssVar`/`getGaugeTailwindClass`가 남아 있으나 실제 사용처가 없다 (dead code 후보). 색상 SSOT는 `CHANNEL_STEPS`이며, 레거시 팔레트와 색상값이 상이하므로 제거 또는 통합 대상이다.
...
| 태그 | 값 | CSS 변수 | 색상 | 사용처 |
|------|-----|----------|------|--------|
| 혼밥/혼술 | `solo` | `--scene-solo` | `#7A9BAE` (슬레이트) | 식당 · 와인 (공용) |
| 데이트 | `romantic` | `--scene-romantic` | `#B8879B` (더스티 로즈) | 식당 · 와인 (공용) |
| 친구/모임 | `friends` | `--scene-friends` | `#7EAE8B` (세이지) | 식당 전용 (와인은 `gathering`이 재사용) |
| 가족 | `family` | `--scene-family` | `#C9A96E` (머스타드) | 식당 전용 |
| 회식/접대 | `business` | `--scene-business` | `#8B7396` (모브) | 식당 전용 (와인은 `gift`가 재사용) |
| 술자리 | `drinks` | `--scene-drinks` | `#B87272` (로즈우드) | 식당 전용 (와인은 `tasting`이 재사용) |
| 디캔팅 | `decanting` | `--scene-decanting` | `#A0896C` (웜 브라운) | 와인 전용 |
...
| 테이스팅 | `tasting` | `--scene-drinks` (#B87272) |
| 디캔팅 | `decanting` | `#A0896C` (웜 브라운) |

> ※ 와인 scene은 위 5종(`pairing/gathering/gift/tasting/decanting`)에 식당에서 재사용하는 `solo/romantic` 2종을 더해 **실제 7종**이다. 와인 `solo`/`romantic`은 식당 `--scene-solo`/`--scene-romantic`을 그대로 재사용한다 (DATA_MODEL §8 와인 scene 7종 참조).
...
> **레거시 주의**: 찜(bookmark/wishlist) 버튼 UI는 migration 063에서 제거됨. 기존 `heart` 아이콘은 리액션(good) 전용으로 재정의되었다.
...
Tailwind CSS v4에서는 `tailwind.config.ts` 없이 `globals.css`의 `@theme` 블록으로 커스텀 토큰을 정의한다. **`tailwind.config.ts`는 존재하지 않는다.**

#### 폰트
| Tailwind | `@theme` 변수 | 용도 |
|----------|-------------|------|
| `font-sans` | `--font-family-sans` | 본문 (Pretendard) |
| `font-comfortaa` | `--font-family-comfortaa` | 로고 |

#### 컬러 (주요)
| Tailwind | `@theme` → `:root` | 예시 |
|----------|-------------------|------|
| `bg-background`, `text-background` | `--color-background` → `var(--bg)` | 기본 배경 |
| `bg-foreground`, `text-foreground` | `--color-foreground` → `var(--text)` | 기본 텍스트 |
| `bg-card`, `text-card` | `--color-card` → `var(--bg-card)` | 카드 |
| `bg-elevated` | `--color-elevated` → `var(--bg-elevated)` | 모달/시트 |
| `bg-page` | `--color-page` → `var(--bg-page)` | 프레임 밖 |
| `text-text-sub` | `--color-text-sub` → `var(--text-sub)` | 보조 텍스트 |
| `text-text-hint` | `--color-text-hint` → `var(--text-hint)` | 힌트 |
| `text-text-inverse` | `--color-text-inverse` → `var(--text-inverse)` | 반전 텍스트 |
| `border-border`, `border-border-bold` | `--color-border`, `--color-border-bold` | 보더 |
| `text-brand`, `bg-brand` | `--color-brand` → `var(--brand)` | 브랜드 전용 |
| `bg-accent-food`, `bg-accent-food-light`, `bg-accent-food-dim` | `--color-accent-food*` | 식당 |
| `bg-accent-wine`, `bg-accent-wine-light`, `bg-accent-wine-dim` | `--color-accent-wine*` | 와인 |
| `bg-accent-social`, `bg-accent-social-light` | `--color-accent-social*` | 소셜 |
| `text-positive`, `text-caution`, `text-negative` | `--color-positive`, `--color-caution`, `--color-negative` | 시맨틱 |
| `text-gauge-food-min`/`max`, `text-gauge-exp-min`/`max`, `text-gauge-total-min`/`max` | `--color-gauge-*` | 게이지 (식당) |

> **주의**: `--gauge-wine-min`/`--gauge-wine-max`는 `:root`에만 존재하고 `@theme` 매핑이 없다. Tailwind 유틸리티(`text-gauge-wine-min` 등)로는 접근 불가하며, `var(--gauge-wine-min)` 형태로 CSS 변수로만 사용한다. 실제 색상 선택은 전적으로 `shared/utils/gauge-color.ts`의 `CHANNEL_STEPS`에서 관리.

#### 반경 / 그림자
| Tailwind | `@theme` 변수 |
|----------|-------------|
| `rounded-xs`, `rounded-sm`, `rounded-md`, `rounded-lg`, `rounded-xl`, `rounded-full` | `--radius-xs`~`--radius-full` → `--r-*` |
| `shadow-sm`, `shadow-md`, `shadow-lg`, `shadow-sheet` | `--shadow-*` |

#### shadcn/ui 호환
`:root`에 `--primary`, `--secondary`, `--muted`, `--accent`, `--destructive`, `--popover`, `--ring`, `--radius` 등을 추가로 선언하되, shadcn 컴포넌트가 참조할 때에만 사용. **일반 UI 작성 시에는 위 `@theme` 매핑(또는 `var(--*)` 직접 참조)을 우선 사용한다.**
...
실제 구현은 `src/presentation/components/ui/`에 있다. 스펙 ↔ 파일 매핑:

| 스펙 섹션 | CSS 클래스 | 구현 파일 (`src/presentation/components/ui/`) |
|----------|-----------|---------------------------------------|
| 버튼 | `.btn-primary`, `.btn-ghost`, `.btn-card-action`, `.btn-social`, `.btn-skip`, `.btn-search-submit` | `button.tsx` |
| 카드 | `.card`, `.card.visited`, `.card.confirmed` | `nyam-card.tsx` |
| 만족도 게이지 | `.satisfaction-gauge*` | `gauge-slider.tsx` (+ `record/satisfaction-gauge.tsx`) |
| Circle Rating | — (JS 구현) | `record/circle-rating.tsx` |
| 태그 | `.tag`, `.tag.food`, `.tag.wine` | `tag.tsx` |
| 씬 태그 | `.scene-tag` | `scene-tag.tsx` |
| 와인 칩 | `.wine-chip.*` | `wine-chip.tsx` |
| 뱃지 | `.badge.*` | `badge.tsx`, `prestige-badges.tsx` |
| 필터 시스템 | `.icon-button`, `.ds-filter-drawer`, `.filter-chip`, `.ds-sort-dropdown`, `.ds-search-dropdown` | `filter-system.tsx`, `filter-chip.tsx`, `icon-button.tsx`, `search-dropdown.tsx` |
| 드롭다운 셀렉트 | `.nyam-select*`, `.nyam-dropdown*` | `nyam-select.tsx` |
| 필터 탭 | `.filter-tab.*` | `filter-tab.tsx` |
| 뷰 사이클 | `.view-cycle-btn` | `view-cycle-button.tsx` |
| 컴팩트 리스트 | `.compact-item`, `.compact-thumb`, `.compact-name`, `.compact-score.*` | `compact-list-item.tsx` |
| 토글 | `.toggle` | `nyam-toggle.tsx` |
| 세그먼트 | `.prv-segment*` | `segment.tsx` |
| 인풋 | `.nyam-input` | `nyam-input.tsx` |
| 바텀 시트 | `.bottom-sheet*`, `.popup-window-overlay` | `bottom-sheet.tsx`, `popup-window.tsx` |
| 알림 드롭다운 | `.notif-*` | `notification/` 하위 (공용 UI 외부) |
| 스텝 프로그레스 | `.step-progress`, `.step-bar.*` | `step-progress.tsx` |
| 인트로 카드 | `.intro-card*` | `intro-card.tsx` |
| 빈 상태 | `.empty-state*` | `empty-state.tsx` |
| 토스트 | `.toast*` | `toast.tsx` |
| 로딩 | `.loading-state` | `loading-state.tsx` |
| 알림 다이얼로그 | — | `alert-dialog.tsx` |
| 사진 뷰어 | — | `photo-viewer.tsx` |
| 스티키 탭 | — | `sticky-tabs.tsx` |

> Floating 버튼(`.fab-back`, `.fab-add`, `.fab-forward`, `.fab-actions`)은 공용 UI 파일 없이 globals.css 클래스로 직접 사용.
...
| ~~**Card Action 활성 (찜)**~~ | ~~`--caution` 배경, white 텍스트~~ | — | — | 레거시 (migration 063에서 제거됨) |
...
| ~~찜~~ | ~~`--caution`~~ | ~~`#FBF8F1`~~ (레거시 — migration 063에서 제거됨) |
...
- 활성: `--accent-food` 배경+보더, `--text-inverse` 텍스트
- 와인 활성: `--accent-wine` 배경+보더
- 소셜 활성: `--accent-social` 배경+보더
- 카운트 표시: `.filter-chip-count` (weight 500, opacity 0.5 → active 시 0.7)
- 가로 스크롤 가능 (`overflow-x: auto`, 스크롤바 숨김)

**아이콘 버튼 상태 변형** (`.icon-button`):
- 기본: `--text-hint`
- `.active`: `--accent-food`
- `.active.wine`: `--accent-wine`
- `.active.map` / `.active.social`: `--accent-social`
...
- 설명: 12px `--text-hint`
- 구현: `src/presentation/components/ui/empty-state.tsx`

### 통계 패널 전용 패턴

`src/presentation/components/home/home-stats-panel.tsx` 내부 파일 스코프 컴포넌트로 존재 (공용 UI 아님).
홈 통계 모드(`StatsToggle`)에서만 사용하며 다른 페이지 재사용 금지 — 3회 이상 반복되는 시점에만 `ui/`로 승격.

| 컴포넌트 | 역할 | 스타일 |
|---------|------|--------|
| `SummaryCard` | 요약 그리드 (총 기록, 평균 점수, 국가 수 등) | `bg-card` 배경, `border-border`, `rounded-lg`, 그리드 내부 `text-h2`/`text-caption` 조합. `accentColor`는 채널 색상(food/wine)을 인라인 전달 |
| `SectionCard` | 아이콘 + 타이틀 + 본문 차트 카드 | 상단 `flex gap-2`에 15px Lucide 아이콘(accentColor) + 13px/700 타이틀, 본문은 children. 카드 자체가 제목 렌더링을 담당하므로 내부 차트(WorldMapChart 등)는 제목 중복 표시 금지 |
| `StatsEmpty` | 기록 부족 시 안내 (식당/와인 공통) | `EmptyState`와 유사 스타일, `type: 'restaurant' | 'wine'`에 따라 문구 교체. `stats.totalRecordCount === 0`이면 즉시 렌더 |

> **진입 조건**: 문서 `RECORD_SYSTEM.md`의 통계 임계값을 따른다 (현재 임계값 1, WORKLOG #55 기준).
> **다크모드**: `accentColor`는 CSS 변수로 전달하므로 자동 대응.
...
| 키프레임 | 용도 | 타이밍 |
|---------|------|--------|
| ~~`wishlist-add-anim`~~ | ~~찜 추가 (scale 0→1.2→1)~~ | 레거시 (migration 063에서 제거됨) |
| ~~`wishlist-remove-anim`~~ | ~~찜 제거 (scale 1→0.8→1)~~ | 레거시 (migration 063에서 제거됨) |
...
| 컨텍스트 | 파일 | 점 크기 | 상수 |
|---------|------|--------|------|
| 기록 상세 참조점 (다른 기록 dot) | `src/presentation/components/record/quadrant-ref-dot.tsx` | **고정 20px** | `DOT_SIZE = 20` |
| 기록 상세 미니 사분면 (현재 기록) | `src/presentation/components/record/mini-quadrant.tsx` | **고정 20px** | `CURRENT_DOT_SIZE = 20` |
| 기록 상세 미니 사분면 (참조 기록) | `src/presentation/components/record/mini-quadrant.tsx` | **고정 14px** | `REF_DOT_SIZE = 14` |
| 홈 카드 미니 사분면 | `src/presentation/components/home/mini-quadrant.tsx` | **고정 6px** | `DOT_SIZE = 6` |
...
> ※ 칭호/구간 SSOT는 `XP_SYSTEM.md` §5. 본 §10은 색상 토큰 매핑 전담. 두 문서 드리프트 방지를 위해 색상 변경 시 양쪽 동기화 필수.

> ※ `src/shared/utils/level-color.ts` (`getLevelColor`/`getLevelCssVar`/`getLevelTailwindClass`)는 실재하지만 현재 코드베이스 어디에서도 import되지 않는 **dead code — 제거 TODO**. 레벨 배지/XP 필 등 실사용처는 위 표의 CSS 변수(`--positive`/`--accent-social`/`--accent-wine`/`--accent-food`/`--caution`)를 직접 참조한다.
...
| ~~찜~~ | ~~`#2E2A1E`~~ | ~~`#5A4A2A`~~ (레거시 — migration 063에서 제거됨) |
...
디자인 토큰은 아니지만, 사진 업로드 품질·크기를 중앙화하여 관리하는 상수.

**위치**: `src/domain/entities/record-photo.ts` — `PHOTO_CONSTANTS`
...
| 상수 | 값 | 설명 |
|------|-----|------|
| `MAX_WIDTH` | 800 | canvas 리사이즈 최대 가로 (px). 모든 업로드 경로에서 공통 |
| `QUALITY` | 0.7 | `canvas.toBlob` WebP 품질 |
| `OUTPUT_FORMAT` | `'image/webp'` | 출력 포맷 |
| `MAX_PHOTOS` | 10 | 기록당 사진 최대 |
| `MAX_FILE_SIZE` | 10MB | 업로드 전 파일 크기 상한 |

**원칙**: `photo-crop-editor.tsx`, `photo-picker.tsx`, `image-upload.ts`, `use-photo-upload.ts` 등 모든 경로가 이 상수만 참조. 품질/크기 조정이 필요하면 이 한 파일만 수정.

**관련 상수**: `src/domain/entities/bubble-photo.ts`에 버블 아이콘용 별도 상수가 존재.
