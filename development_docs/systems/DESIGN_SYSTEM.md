...
**Worst → Best 2색 연속 그라데이션**. 점수에 따라 선형 보간.

```css
--gauge-worst: #C4B5A8;  /* 웜그레이 — 낮은 만족 */
--gauge-best:  #7A9BAE;  /* 슬레이트 — 최고 */
```

| 점수 | 색상 | 느낌 |
|------|------|------|
| ~20 | `#C4B5A8` | Worst (따뜻한 회색) |
| 50 | `#9FA5A3` | 중간 (뉴트럴) |
| 80 | `#889DAB` | 좋음 |
| 100 | `#7A9BAE` | Best (차분한 블루) |

- 점수 20~100을 0~1로 매핑, RGB 선형 보간
- 이산 색상 분기 없음 — 점수 변화에 따라 자연스러운 색 전환
- 사분면 점, 게이지 바, 카드 인라인 점수 표시에 공통 적용
...
| 속성 | 메인 헤더 | 내부 페이지 헤더 |
|------|----------|----------------|
| 왼쪽 | nyam 로고 (Comfortaa 22px) | `chevron-left` 22px + 이전 페이지명 16px/600 |
...
| 페이지 | 이전 페이지명 | 파일 |
|--------|-------------|------|
| 버블 상세 | 버블 | 04_bubbles, 03_profile |
| 버블 만들기 | 버블 | 04_bubbles, 03_profile |
| 버블 설정 | {버블명} | 04_bubbles |
| 프로필 공유 (Wrapped) | 프로필 | 03_profile |
| 버블 목록 (프로필 내) | 프로필 | 03_profile |
| 알림 | 프로필 | 03_profile |
| 설정 | 프로필 | 05_settings |
| 식당/와인 상세 | fab-back만 (히어로 카루셀) | 02_detail |
