# S2: Core Recording — 사분면 + 아로마 + 구조평가 + 기록 플로우
...
| 문서 | 역할 |
|------|------|
| `systems/RATING_ENGINE.md` | 사분면 좌표, 만족도, 씬태그, 아로마 팔레트, 구조평가, 페어링 |
| `systems/DATA_MODEL.md` | records 테이블 스키마 (domain 엔티티 출처), record_photos |
...
- [ ] Record, Rating, Quadrant domain 엔티티
- [ ] 사분면 UI (식당: 가격x분위기, 와인: 산미x바디)
- [ ] 만족도 게이지 (1-100) + Circle Rating 인터랙션
- [ ] 아로마 팔레트 (와인 15섹터 3링 — 1차향 8 + 2차향 4 + 3차향 3)
- [ ] 와인 구조평가 (복합성/여운/균형 슬라이더 3개, 선택)
- [ ] 페어링 카테고리 (WSET 8종 그리드 + AI 추천 + 직접 입력)
- [ ] 씬태그 (식당 6종 단일 선택) + 동행자 선택 UI
- [ ] 사진 업로드 UI + Supabase Storage 연결 (record_photos)
- [ ] 기록 저장 플로우 (Phase 1 필수 + Phase 2 선택)
...
| 2.4 | 아로마 팔레트 (15섹터 3링) | `04_aroma.md` | 2.1 |
| 2.5 | 와인 구조평가 (슬라이더 3개) | `05_structure.md` | 2.1 |
...
```
Step 1: 사분면 (산미×바디) + 만족도       ← 필수
Step 2: 아로마 팔레트 (15섹터 3링)        ← 필수
Step 3: 구조평가 (복합성/여운/균형)        ← 선택
...
```
□ 사분면 터치/드래그 → 좌표 정확히 저장 (위치 이동 + 만족도 조절 독립 제스처)
□ Circle Rating: 28px~120px 크기 범위, 5단계 게이지 색상, glow 효과
□ 만족도 1~100 범위 제한, 10점 단위 햅틱
□ 아로마 15섹터 3링: 탭 토글, 드래그 연속 칠하기, aroma_color 가중평균 계산
□ 구조평가: 복합성/여운/균형 슬라이더 → 만족도 자동산출 (수동 우선)
...
□ Phase 1 저장 → status='rated', Phase 2 부가 정보 업데이트
□ 저장된 기록 조회 시 모든 필드 정확히 표시
□ (검색/목록 경로 → status='checked'는 S3에서 검증)
```
