...
### DEC-007: 사진 저장 경로 — Container 오케스트레이션 + Graceful Degradation
- **일시**: 2026-03-28
- **스프린트**: S2 (2.9)
- **맥락**: 설계 문서(`09_record_flow.md §8`)는 `useCreateRecord` hook 내부에서 records INSERT → record_photos INSERT → wishlists UPDATE를 일괄 처리하도록 명시했다. 그러나 실제 구현에서는 사진 업로드(리사이즈 + Storage)를 담당하는 `usePhotoUpload` hook이 별도 존재하며, container에서 오케스트레이션하는 패턴으로 구현되었다.
- **선택지**:
  - (A) Hook 통합 — `useCreateRecord`가 record + photo + wishlist 전부 처리. 원자성 강함, 단일 try/catch
  - (B) Container 오케스트레이션 — record hook, photo hook, photoRepo를 container가 순서대로 호출. 관심사 분리, graceful degradation
- **결정**: (B) Container 오케스트레이션 — 이유:
  1. 관심사 분리: record 저장, 사진 업로드, 사진 DB 저장이 각각 독립
  2. Graceful degradation: 사진 업로드 실패 시 record는 유지하고 "사진 업로드 실패" 메시지만 표시. 사용자가 상세 페이지에서 나중에 추가 가능
  3. record 롤백이 오히려 UX 악화: 사분면/아로마/코멘트 등 사용자 입력 전부 날아감
  4. `RecordRepository.addPhotos()` 데드코드 제거, `PhotoRepository.savePhotos()`로 단일 경로 통일
- **영향**: `09_record_flow.md §8` 갱신, `RecordRepository.addPhotos()` 인터페이스/구현 제거, `record-success.tsx`에 photoError prop 추가
- **되돌림 가능 여부**: 쉬움

### DEC-006: Application Hook DI 패턴 — Service Locator 채택
- **일시**: 2026-03-28
- **스프린트**: S2 (전체 적용)
- **맥락**: 설계 문서(`10_infra.md`, `CLEAN_ARCH_PATTERN.md`)는 hook이 repository를 파라미터로 받는 Constructor Injection 패턴을 명시했다 (`useCreateRecord(repository: RecordRepository)`). 그러나 실제 구현 과정에서 프로젝트 전체 50개 hook이 `shared/di/container`에서 직접 import하는 Service Locator 패턴으로 통일되었다.
- **선택지**:
  - (A) Constructor Injection — `useHook(repo: Repository)` → container에서 주입. 테스트 시 mock 주입 용이
  - (B) Service Locator — hook이 `shared/di/container` 직접 import. 호출 코드 간결, container 보일러플레이트 감소
- **결정**: (B) Service Locator — 이유:
  1. 프로젝트 전체 50개 hook이 이미 이 패턴으로 수렴 (일관성)
  2. `shared/di`는 infrastructure가 아니므로 R3 위반이 아님
  3. container.ts에서 export 시 타입이 domain 인터페이스로 좁혀져 있어 의존성 역전 원칙 준수
  4. 현재 hook 단위 테스트가 없어 Constructor Injection의 테스트 용이성 이점 불명확
- **영향**: `10_infra.md` §5~§6, `CLEAN_ARCH_PATTERN.md` DI 패턴 섹션 갱신 완료. 향후 hook 작성 시 이 패턴을 따른다.
- **되돌림 가능 여부**: 쉬움 — hook 테스트 도입 시 Constructor Injection으로 전환 가능
...
### D-011 홈 메인 화면 목업 매칭 (01_home.html)

**일시**: 2026-03-28
**맥락**: 01_home.html 목업 vs 코드 Playwright 비교 결과 7가지 불일치 발견
**결정**:
1. `(main)/layout.tsx`의 임시 헤더 제거 → AppHeader가 이미 HomeContainer에 포함되어 중복
2. HomeTabs: pill 스타일을 밑줄(underline) active로 변경 — 목업이 명확히 밑줄 스타일
3. RecordCard: 단순 카드 → place-card 구조 전면 재작성 (MiniQuadrant, SourceTag, badges, engagement)
4. AiGreeting/NudgeStrip: 기존 컴포넌트 인터페이스를 목업 구조에 맞춰 재작성 (props 변경)

**수정 파일**: app-header.tsx, ai-greeting.tsx, nudge-strip.tsx, home-tabs.tsx, record-card.tsx, home-container.tsx, (main)/layout.tsx
**검증**: Playwright 390×844 스크린샷 → 목업과 시각적 일치 확인, pnpm build/lint 0 errors
