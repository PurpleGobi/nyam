...
### Wave 7 — Drill-down Maps (1 세션)
> 선행: S6 완료 (프로필 페이지 존재). S9 완료 후 진행 권장.

| 세션 | 작업 | 완료 조건 |
|------|------|----------|
| **A** | S10 전체 (SVG 지도 데이터 + 식당 드릴다운 + 와인 3단계 드릴다운) | 두 지도 모두 드릴다운 동작 + 360px 정상 |

---

## 웨이브 진행 현황

| Wave | 스프린트 | 세션 수 | 상태 | 시작일 | 완료일 |
|------|---------|---------|------|--------|--------|
...
| # | 태스크 | 지침 문서 | 상태 | 검증 |
|---|--------|----------|------|------|
| 6.1 | XP 계산 엔진 + 활성 XP 크론 | `S6/01_xp_engine.md` | `done` | xp entity+calculator+repo+supabase repo+useXp+useXpAward+Edge Function ✅ |
| 6.2 | 프로필 페이지 (통계 + 맛 정체성) | `S6/02_profile.md` | `done` | profile entity+repo+supabase repo+useProfileStats+useWrapped+11 UI컴포넌트+/profile/wrapped ✅ (지도 드릴다운은 S10 위임) |
| 6.3 | 설정 페이지 | `S6/03_settings.md` | `done` | settings-repo+supabase-settings-repo+12 UI컴포넌트(section/card/item/toggle/segment/privacy-layer/privacy-summary/privacy-note/sheets)+NyamSelect ✅ |
...
### S10 — Drill-down Maps (Wave 7)
| # | 태스크 | 지침 문서 | 상태 | 검증 |
|---|--------|----------|------|------|
| 10.1 | SVG 지도 데이터 준비 | `S10/01_map_data.md` | `not_started` | — |
| 10.2 | 식당 드릴다운 지도 | `S10/02_restaurant_map.md` | `not_started` | — |
| 10.3 | 와인 산지 드릴다운 지도 | `S10/03_wine_region_map.md` | `not_started` | — |
| 10.4 | S10 검증 | `S10/04_validation.md` | `not_started` | — |

---

## 스프린트 구조 요약 (10 스프린트, 68 태스크)
