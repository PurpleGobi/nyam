...
XP_BY_QUALITY = {"name": 1, "score": 3, "photo": 10, "full": 15}
...
def xp_to_level(xp: int) -> int:
    ...
    - 중반 꾸준히 올라감 (성취감 유지)
    - 고레벨(80+)만 느려짐 (희소성)
    - 파워유저 2년(XP ~7,500) → Lv.70~80
    - 캐주얼 2년(XP ~200) → Lv.15~20
    """
    if xp <= 0:
        return 1
    ...
    # 목표: 게임식 커브
    #   초반 쉬움 → 점점 어려워짐
    #   파워유저: 1년=Lv.50, 2년=Lv.70, 3년=Lv.80, 4년=Lv.86
    #   캐주얼:  1년=Lv.10, 2년=Lv.17
    #   Lv.99 = 전설 (5년+ 극한 파워유저)
    #
    # 파워유저 XP 추정: 1년~3,700, 2년~7,500, 3년~12,000, 4년~16,000
    thresholds = []
    cumul = 0
    req = 3      # Lv.2: 단 3 XP
    for lv in range(2, 100):
        ...
        if lv <= 10:
            req = max(int(req + 1), req + 1)       # +1 선형: 3,4,5,...12 → 빠름
        elif lv <= 30:
            req = max(int(req * 1.06), req + 1)    # 6% 증가: 보통
        elif lv <= 50:
            req = max(int(req * 1.07), req + 2)    # 7% 증가: 느려짐
        elif lv <= 70:
            req = max(int(req * 1.08), req + 3)    # 8% 증가: 꽤 느림
        elif lv <= 85:
            req = max(int(req * 1.12), req + 5)    # 12% 증가: 매우 느림
        else:
            req = max(int(req * 1.15), req + 10)   # 15% 증가: 극도로 느림
...
def run():
    ...
    print(f"  {'─'*80}")
    rows = [
        ("식당/와인 이름만 등록", 1, "종합 XP", ""),
        ("+ 사분면 점수", 3, "종합 XP", ""),
        ("+ 사진 (EXIF GPS 검증)", 10, "종합 XP", "실제 방문 증거"),
        ("+ 풀 기록 (리뷰+상황+메뉴)", 15, "종합 XP", ""),
        ("", 0, "", ""),
        ...
        ("팔로워 획득 (버블/개인)", 2, "종합 XP", "일 10 상한"),
        ("맞팔 성사", 3, "종합 XP", "일 10 상한, 양쪽"),
        ("", 0, "", ""),
        ("온보딩 완료", 10, "종합 XP", "1회"),
        ("첫 기록", 5, "종합 XP", "1회"),
        ("첫 버블 생성", 5, "종합 XP", "1회"),
