...
**3개 카드(나/Nyam/버블)를 나란히 표시. 각 카드 클릭 시 상세 패널 펼침.**
...
| 카드 | 라벨 | 점수 색상 | 부가 텍스트 | 클릭 동작 |
|------|------|----------|------------|----------|
| **나** | "내 점수" | `var(--accent-food)` | "N회 방문" 또는 "미방문" | 없음 |
| **Nyam** | "Nyam" | `var(--primary)` | "확신 XX%" | breakdown 패널 토글 |
| **버블** | "버블" | `var(--accent-social)` | "확신 XX%" + 버블명 | 버블 목록 패널 토글 |

- Nyam/버블 점수는 항상 표시 (숨기지 않음). 확신도를 %로 투명 공개.
- 버블 카드에는 확신도가 가장 높은 버블의 점수를 대표로 표시.
- 가입한 버블 중 기록 1건+ 있는 것만 표시.

#### Nyam breakdown 패널

**Nyam 카드 클릭 시 토글.** 점수 카드 아래에 위치.

```
컨테이너: padding 0 20px 10px
열림: maxHeight 250px, overflow-y auto
...
내용:
- "평가자 N명 기반"
- 팔로잉 기여자 (가중 ×1.5): @닉네임, 적합도 XX%, 점수
- 유사 유저: N명, 평균 적합도 XX%, 평균 점수

#### 버블 확장 패널

**버블 카드 클릭 시 토글.** 점수 카드 아래에 위치.

```
컨테이너: padding 0 20px 10px
열림: maxHeight 200px, overflow-y auto
닫힘: maxHeight 0, overflow hidden
트랜지션: max-height 0.25s ease
```

내용: 가입한 버블별 — 버블명, 점수, 확신 XX%, N명 평가
...
| **ScoreCards** | 3슬롯(나/Nyam/버블) 유지. 버블 카드에 해당 버블 점수가 대표로 표시됨 |
