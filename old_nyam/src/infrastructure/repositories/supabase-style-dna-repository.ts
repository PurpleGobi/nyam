...
function toRegion(row: RegionRow): StyleDnaRegion {
  return {
    userId: row.user_id,
    region: row.region,
...
function toGenre(row: GenreRow): StyleDnaGenre {
  return {
    userId: row.user_id,
...
function toScene(row: SceneRow): StyleDnaScene {
  return {
    userId: row.user_id,
    scene: row.scene,
    recordCount: row.record_count,
...
export class SupabaseStyleDnaRepository implements StyleDnaRepository {
  async getRegionsByUserId(userId: string): Promise<StyleDnaRegion[]> {
  ...
  async getGenresByUserId(userId: string): Promise<StyleDnaGenre[]> {
    const supabase = createClient()
    const { data, error } = await supabase
  ...
  async getScenesByUserId(userId: string): Promise<StyleDnaScene[]> {
    const supabase = createClient()
    const { data, error } = await supabase
